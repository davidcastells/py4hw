...
output_dir = f'/tmp/vitis_hil_{os.getlogin()}/test_fpadder_sp'
...
class Test_FPAdder_SP:
    ...
    def test_create(self):
        ...
        dut = py4hw.FPAdder_SP(sys, "add", a,b, r)
        ...
        if not(os.path.exists(xclbin_path)):
            ...
            msg = f'FPAdder_SP compilation time: {lap} seconds = {hours} : {minutes:02d} : {seconds:02d}\n'

            with open(os.path.join(output_dir, "compilation_time.txt"), "w", encoding="utf-8") as file:
                file.write(msg)
            ...
            with open(os.path.join(output_dir, "compilation_time.txt"), "r", encoding="utf-8") as file:
                content = file.read()
    ...
    def disabled_test_random(self):
        ...
        dut = py4hw.FPAdder_SP(hw, "abs", a,b, r)
    ...
    def test_speedup(self):
        ...
        lap = tf-t0
        sps = N/lap
        print(f'SW = Run {N} tests in {lap} seconds => {sps} samples / second')	

        proxy_ins  = [pin.wire for pin in dut.inPorts]
        # fresh output wires for the proxy, matching the DUT's output widths
        proxy_outs = [pout.wire for pout in dut.outPorts]

        for w in proxy_outs:
            # disconnects the source of the wire
            w.source = None
        ...
        lap = tf-t0
        sps = N/lap
        print(f'HW = Run {N} tests in {lap} seconds => {sps} samples / second')	
