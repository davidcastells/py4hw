...
class I2CMasterWriter(Logic):
    def __init__(self, parent, name, reset, dev_address, dev_reg, dev_data, 
                 ...
                 i2c_sda_in, i2c_sda_out, i2c_sda_oe, i2c_scl):
    ...
    def clock(self):
        ...
        if (self.reset.get() == 1):
            self.state = 0
            self.count = 0
            self.i2c_scl.prepare(1) 
            self.i2c_sda_oe.prepare(0) # do not drive sda
            self.write_busy.prepare(0)
            self.write_error.prepare(0)
        else:
            if (self.state == 0):
                if ((self.write_start.get() == 1) and (self.i2c_sda_in.get() == 1)):
                    # bus available
                    self.state = 1
                    self.i2c_scl.prepare(1) 
                    self.i2c_sda_oe.prepare(1)
                    self.i2c_sda_out.prepare(0)
                ...
                self.i2c_scl.prepare(0) 
                self.i2c_sda_oe.prepare(1)
                self.i2c_sda_out.prepare(self.v_address >> 6)
                self.state = 2
                ...
                if (self.i2c_sda_in.get()):
                    ...
                    self.i2c_sda_oe.prepare(1)
                    self.i2c_sda_out.prepare(self.v_reg >> 7)
                    self.count = 0
                    self.state = 9
                ...
                self.i2c_scl.prepare(0) 
                self.i2c_sda_oe.prepare(1)
                self.i2c_sda_out.prepare(self.v_data >> 7)
                self.state = 14
