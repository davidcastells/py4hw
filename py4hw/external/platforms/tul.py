...
class Pynq_Z2(py4hw.HWSystem):
    ...
    def getRaspberryPIInput(self, i):
        
        ii = self.wire(f'ii{i}')
        o = self.wire(f'o{i}')
        
        wbidir = py4hw.FakeWire(f'rpio_{i}_r')
        self.addInOut(f'rpio_{i}_r', self.wire(f'w_rpio_{i}_r'))
                
        from py4hw.external.xilinx.io import TristateBuffer
        
        t = self.wire(f't{i}')
        py4hw.Constant(self, f't{i}', 1, t)
        TristateBuffer(self, f'tbuf{i}', ii, t, o, wbidir)
            
        return o

    def getRaspberryPIOutput(self, i):
        ...
        t = self.wire(f't{i}')
        py4hw.Constant(self, f't{i}', 0, t)
        TristateBuffer(self, f'tbuf{i}', ii, t, o, wbidir)
            
        return ii

    
    # def getRaspberryPIIOs(self):
        
    #     oes = self.wires('oes', 20, 1)
    #     iis = self.wires('iis', 20, 1)
    #     os = self.wires('os', 20, 1)
        
    #     wbidirs = []
        
    #     for i in range(20):
    #         wbidirs.append( py4hw.FakeWire(f'rpi_gpio_tri_io[{i}]'))
        
    #     self.addInOut('rpi_gpio_tri_io', self.wire('rpi_gpio_tri_io', 32))
    ...
    #     for i in range(20):
    #         t = self.wire(f't{i}')
    #         py4hw.Not(self, f't{i}', oes[i], t)
    #         TristateBuffer(self, f'tbuf{i}', iis[i], t, os[i], wbidirs[i])
